...
class TronService {
	...
	async getVaultContract(token = 'USDT', groupId = null) {
		...
		if (!contractEntry) {
			throw new Error(`EscrowVault not found for ${token} on TRON with ${desiredFeePercent}% fee${groupId ? ` (group ${groupId})` : ''}`);
		}

		const contract = await this.tronWeb.contract(ESCROW_VAULT_ABI, contractEntry.address);
	...
	async getTokenTransfers(token = 'USDT', toAddress, fromBlock = 0) {
		await this.init();
		const tokenAddress = token.toUpperCase() === 'USDT' ? config.USDT_TRON : null;
		...
		try {
			...
			return (events || [])
				.filter((ev) => ev && ev.result && (!fromBlock || ev.block_number > fromBlock))
				.map((ev) => {
