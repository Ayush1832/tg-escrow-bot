...
async function detectTokensInContract(contractAddress, provider, fromBlock = null) {
  ...
  try {
    ...
    console.log(`   🔍 Scanning blocks ${startBlock} to ${currentBlock} for token transfers...`);
...
async function main() {
  ...
  if (!withdrawTo || (hasCustomTokens && !ethers.isAddress(withdrawTo))) {
    ...
    if (withdrawTo && !ethers.isAddress(withdrawTo)) {
      ...
      console.error('   Usage: node withdraw-usdt-bsc.js [withdrawal_address] [token_address1] [token_address2] ...');
      process.exit(1);
    }
  ...
  if (hasCustomTokens) {
    ...
    console.log(`💰 Using ${allTokens.length} custom token(s) from command line`);
    for (const token of allTokens) {
      console.log(`   • ${token.address}`);
    }
  } else {
    allTokens = getAllBSCTokens();
    console.log(`💰 Checking ${allTokens.length} tokens: ${allTokens.map(t => t.name).join(', ')}`);
    console.log(`\n💡 Tip: To check specific tokens, provide addresses as arguments:`);
    console.log(`   node scripts/withdraw-usdt-bsc.js [withdrawal_address] [token1] [token2] ...`);
  }
  ...
  for (const contract of contracts) {
    ...
    console.log(`\n${'='.repeat(60)}`);
    console.log(`📋 Contract: ${contract.token} on ${contract.network} (Fee: ${contract.feePercent}%)`);
    console.log(`   Address: ${contractAddress}`);

    // Check if wallet is the owner
    const vaultContract = new ethers.Contract(contractAddress, ESCROW_VAULT_ABI, provider);
    let owner;
    try {
    ...
    for (const token of allTokens) {
      try {
        ...
        if (balance > 0n) {
          ...
          console.log(`   💰 ${tokenName || token.name} (${symbol}): ${balanceFormatted}`);
        }
      } catch (error) {
    ...
    allWithdrawals.push({
      contractAddress,
      vaultContract: new ethers.Contract(contractAddress, ESCROW_VAULT_ABI, wallet),
      tokenBalances,
  ...
  for (const withdrawal of allWithdrawals) {
    ...
    for (const tokenBalance of withdrawal.tokenBalances) {
      console.log(`  • ${tokenBalance.tokenName} (${tokenBalance.symbol}): ${tokenBalance.balanceFormatted}`);
      totalTokens++;
    }
  }
  ...
  for (const withdrawal of allWithdrawals) {
    ...
    for (const tokenBalance of withdrawal.tokenBalances) {
      try {
        console.log(`   💸 Withdrawing ${tokenBalance.balanceFormatted} ${tokenBalance.symbol}...`);
  ...
  for (const result of successful) {
    console.log(`   • ${result.token} (${result.symbol}): ${result.amount} - ${result.txHash}`);
  }
