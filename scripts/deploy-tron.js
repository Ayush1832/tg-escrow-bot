...
async function main() {
  ...
  if (!TRC_PRIVATE_KEY && !HOT_WALLET_PRIVATE_KEY) throw new Error('TRC_PRIVATE_KEY (or HOT_WALLET_PRIVATE_KEY) missing');
  ...
  const privateKey = (TRC_PRIVATE_KEY || HOT_WALLET_PRIVATE_KEY).replace(/^0x/, ''); // Remove 0x if present
  ...
  const artifactsPath = path.join(__dirname, '..', 'artifacts', 'contracts', 'EscrowVault.sol', 'EscrowVault.json');
  ...
  if (fs.existsSync(artifactsPath)) {
    try {
      ...
      throw new Error('Could not read contract bytecode. Please compile the contract first using Hardhat.');
    }
  } else {
    throw new Error(`Contract artifact not found at ${artifactsPath}. Please compile the contract first using: npx hardhat compile`);
  }
  ...
  const contractBytecode = bytecode.startsWith('0x') ? bytecode.slice(2) : bytecode;
  ...
  try {
    ...
    await ContractModel.updateOne(
      { name: 'EscrowVault', token: 'USDT', network: 'TRON', feePercent: feePercent },
      { 
        name: 'EscrowVault', 
        token: 'USDT',
        network: 'TRON',
    ...
    if (error.message && error.message.includes('bandwidth')) {
      console.error(`💡 Tip: Make sure your wallet has enough bandwidth or TRX for energy.`);
    }
    throw error;
  }
  ...
  deployedContracts.forEach((contract) => {
    ...
    console.log(`    Tronscan: https://tronscan.org/#/contract/${contract.address}`);
  });
