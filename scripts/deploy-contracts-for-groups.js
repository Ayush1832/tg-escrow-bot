...
const GroupPool = require(path.join(__dirname, '..', 'src', 'models', 'GroupPool'));
...
async function main() {
  ...
  if (!HOT_WALLET_PRIVATE_KEY) {
    throw new Error('HOT_WALLET_PRIVATE_KEY missing from .env file. This is required for deployment.');
  }
  ...
  if (network.chainId !== 56n) {
    ...
    console.error(`   npx hardhat run scripts/deploy-contracts-for-groups.js --network bsc`);
    process.exit(1);
  }
  ...
  if (existingContracts.length > 0) {
    ...
    console.error('   This script should only be run once. Existing contracts:');
    existingContracts.forEach(contract => {
      console.error(`   • ${contract.token} for group ${contract.groupId}: ${contract.address}`);
    });
  ...
  if (groups.length !== 20) {
    ...
    console.error('   Please ensure you have exactly 20 groups (Room 1-20) in the pool');
    await mongoose.disconnect();
    process.exit(1);
  }
  ...
  if (invalidGroups.length > 0) {
    console.error(`❌ ERROR: Found ${invalidGroups.length} groups without groupId!`);
  ...
  if (groupIds.length !== uniqueGroupIds.length) {
    ...
    console.error(`   Total groups: ${groupIds.length}, Unique groupIds: ${uniqueGroupIds.length}`);
    await mongoose.disconnect();
    process.exit(1);
  }
  ...
  groups.forEach((group, index) => {
    console.log(`   ${index + 1}. ${group.groupTitle || group.groupId} (ID: ${group.groupId})`);
  });
  ...
  if (deployer.address.toLowerCase() !== expectedAddress.toLowerCase()) {
    ...
    console.error(`   Please check your HOT_WALLET_PRIVATE_KEY in .env and hardhat.config.js`);
    await mongoose.disconnect();
    process.exit(1);
  }
  ...
  console.log(`🔑 Private Key: ${HOT_WALLET_PRIVATE_KEY.slice(0, 10)}...${HOT_WALLET_PRIVATE_KEY.slice(-8)} (hidden)`);
  console.log(`💰 Balance: ${balanceBNB} BNB`);
  console.log(`${'='.repeat(60)}\n`);
  
  if (parseFloat(balanceBNB) < 0.1) {
    console.error(`\n❌ WARNING: Low BNB balance (${balanceBNB} BNB)`);
    console.error(`   You may need more BNB for gas fees (recommended: at least 0.5 BNB)`);
    console.error(`   Each contract deployment costs approximately 0.01-0.02 BNB`);
  ...
  for (let i = 0; i < 20; i++) {
    ...
    try {
      ...
      if (existingContract) {
        console.log(`\n[${roomNumber}/20] ⚠️  USDT contract already exists for ${group.groupTitle || group.groupId}`);
      ...
      console.log(`\n[${roomNumber}/20] Deploying USDT contract for ${group.groupTitle || group.groupId}...`);
      console.log(`   Group ID: ${group.groupId}`);
      console.log(`   Token: ${USDT_BSC}`);
      console.log(`   Fee Wallets: ${w1}, ${w2 || w1}`);
      console.log(`   Fee Percent: ${feePercent}% (${feePercent * 100} basis points)`);
      
      // Estimate gas before deploying
      const EscrowVaultFactory = await hre.ethers.getContractFactory('EscrowVault');
      ...
      if (deployedToken.toLowerCase() !== USDT_BSC.toLowerCase()) {
        throw new Error(`Token mismatch: expected ${USDT_BSC}, got ${deployedToken}`);
      }
      if (deployedOwner.toLowerCase() !== deployer.address.toLowerCase()) {
        throw new Error(`Owner mismatch: expected ${deployer.address}, got ${deployedOwner}`);
      }
      if (deployedFeePercent.toString() !== (feePercent * 100).toString()) {
        throw new Error(`Fee mismatch: expected ${feePercent * 100}, got ${deployedFeePercent}`);
      }
      
      console.log(`   ✅ Deployed: ${contractAddress}`);
      console.log(`   ✅ Verified: Token=${deployedToken}, Owner=${deployedOwner}, Fee=${deployedFeePercent}`);
      ...
      try {
        ...
        console.log(`   💾 Saved to database (assigned to ${group.groupTitle || group.groupId})`);
      } catch (dbError) {
        // If database save fails, we still have a deployed contract
        console.error(`   ⚠️  WARNING: Contract deployed but database save failed!`);
      ...
      console.error(`   ❌ Error deploying USDT contract for ${group.groupTitle || group.groupId}:`, error.message);
      if (error.stack) {
  ...
  for (let i = 0; i < 20; i++) {
    ...
    try {
      ...
      if (existingContract) {
        console.log(`\n[${roomNumber}/20] ⚠️  USDC contract already exists for ${group.groupTitle || group.groupId}`);
        console.log(`   Address: ${existingContract.address}`);
        console.log(`   Skipping deployment...`);
        deployedContracts.push({
          token: 'USDC',
      ...
      console.log(`\n[${roomNumber}/20] Deploying USDC contract for ${group.groupTitle || group.groupId}...`);
      console.log(`   Group ID: ${group.groupId}`);
      ...
      if (deployedToken.toLowerCase() !== USDC_BSC.toLowerCase()) {
        throw new Error(`Token mismatch: expected ${USDC_BSC}, got ${deployedToken}`);
      }
      if (deployedOwner.toLowerCase() !== deployer.address.toLowerCase()) {
        throw new Error(`Owner mismatch: expected ${deployer.address}, got ${deployedOwner}`);
      }
      ...
      console.error(`   ❌ Error deploying USDC contract for ${group.groupTitle || group.groupId}:`, error.message);
      if (error.stack) {
        console.error(`   Stack: ${error.stack}`);
      }
      errors.push({
  ...
  console.log(`${'='.repeat(60)}`);
  console.log(`✅ Successfully deployed: ${deployedContracts.length} contracts`);
  ...
  for (let i = 0; i < 20; i++) {
    const group = groups[i];
    const usdtContract = deployedContracts.find(c => c.token === 'USDT' && c.roomNumber === i + 1);
    const usdcContract = deployedContracts.find(c => c.token === 'USDC' && c.roomNumber === i + 1);
    
    console.log(`\n${group.groupTitle || group.groupId}:`);
    if (usdtContract) {
      console.log(`   ✅ USDT: ${usdtContract.address}`);
      console.log(`      🔗 https://bscscan.com/address/${usdtContract.address}`);
    } else {
      console.log(`   ❌ USDT: Failed`);
    }
    if (usdcContract) {
      console.log(`   ✅ USDC: ${usdcContract.address}`);
      console.log(`      🔗 https://bscscan.com/address/${usdcContract.address}`);
    } else {
  ...
  for (const group of groups) {
    const usdtContract = dbContracts.find(c => c.groupId === group.groupId && c.token === 'USDT');
    const usdcContract = dbContracts.find(c => c.groupId === group.groupId && c.token === 'USDC');
    
    if (!usdtContract) {
      missingContracts.push({ group: group.groupTitle || group.groupId, token: 'USDT' });
    }
    if (!usdcContract) {
      missingContracts.push({ group: group.groupTitle || group.groupId, token: 'USDC' });
    }
  }
  ...
  if (missingContracts.length > 0) {
    ...
    console.log(`   ✅ All groups have both USDT and USDC contracts in database`);
  }
  ...
  console.log(`   4. The system will automatically use group-specific contracts`);
