...
async function main() {
  ...
  if (!adminAddress) {
    ...
    if (HOT_WALLET_PRIVATE_KEY) {
      ...
      console.error('❌ ADMIN_WALLET_ADDRESS or HOT_WALLET_PRIVATE_KEY must be set');
      process.exit(1);
    }
  }
  ...
  console.log('='.repeat(80));
  console.log(`📊 Found ${contractsWithBalance.length} contract(s) with balance`);
  console.log('='.repeat(80));
  console.log('');
  ...
  const totalAmount = contractsWithBalance.reduce((sum, item) => sum + item.balance, 0);
  console.log(`⚠️  WARNING: This will withdraw all funds from ${contractsWithBalance.length} contract(s)`);
  ...
  for (let i = 0; i < contractsWithBalance.length; i++) {
    ...
    try {
      console.log(`${i + 1}/${contractsWithBalance.length}. Withdrawing from ${token} on ${network.toUpperCase()}`);
      console.log(`   Contract: ${contractAddress}`);
  ...
  if (successCount > 0) {
    ...
    results.filter(r => r.status === 'success').forEach((result, idx) => {
      ...
      console.log(`      Amount: ${result.amount.toFixed(6)} ${result.token}`);
