...
const BlockchainService = require(path.join('..', 'src', 'services', 'BlockchainService'));
...
async function main() {
  const { MONGODB_URI, BSC_RPC_URL, HOT_WALLET_PRIVATE_KEY, USDT_BSC } = process.env;
  ...
  const walletBalanceFormatted = ethers.formatUnits(walletBalance, usdtDecimals);
  console.log(`💰 Wallet Address: ${wallet.address}`);
  console.log(`💰 Wallet USDT Balance: ${walletBalanceFormatted} USDT\n`);

  // Amount to transfer per contract (0.1 USDT)
  const transferAmount = 0.1;
  const transferAmountWei = ethers.parseUnits(transferAmount.toString(), usdtDecimals);
  ...
  const totalNeededWei = ethers.parseUnits(totalNeeded.toString(), usdtDecimals);
  ...
  if (walletBalance < totalNeededWei) {
    console.error(`❌ Insufficient balance! Need ${totalNeeded} USDT but have ${walletBalanceFormatted} USDT`);
    await mongoose.disconnect();
    process.exit(1);
  }
  ...
  console.log(`⚠️  WARNING: This will transfer ${transferAmount} USDT to ${contracts.length} contract(s)`);
  ...
  for (let i = 0; i < contracts.length; i++) {
    ...
    try {
      console.log(`${i + 1}/${contracts.length}. Transferring to ${token} on ${network.toUpperCase()}`);
      ...
      // Transfer USDT
      const tx = await usdtContract.transfer(contractAddress, transferAmountWei, {
        nonce: nonce
      });
  ...
  console.log(`💰 Total transferred: ${(successCount * transferAmount).toFixed(6)} USDT`);
  console.log('');

  if (successCount > 0) {
    console.log('✅ Successful Transfers:');
    results.filter(r => r.status === 'success').forEach((result, idx) => {
      console.log(`   ${idx + 1}. ${result.token} on ${result.network.toUpperCase()}`);
  ...
  if (failCount > 0) {
    ...
    results.filter(r => r.status === 'failed').forEach((result, idx) => {
      console.log(`   ${idx + 1}. ${result.token} on ${result.network.toUpperCase()}`);
      console.log(`      Contract: ${result.contract}`);
      console.log(`      Amount: ${result.amount} USDT`);
